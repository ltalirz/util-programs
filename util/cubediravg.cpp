...
bool calculate(types::String cubeFile, types::Uint dir);
...
int main(int ac, char* av[]) {
    ...
    if (parse(ac, av, args)) {
        ...
        calculate(args["cubefile"].as< types::String >(),  dir);
    }
...
bool calculate(types::String cubeFile, types::Uint dir) {
    ...
    c.grid.dirAverage(dir, plane);
    outFile += ".lineaverage";
    std::cout << "Writing line average to " << outFile << "\n";
...
bool parse(int ac, char* av[], po::variables_map& vm) {
    ...
    if (    !vm.count("cubefile")) {
        std::cout << "Usage: cubediravg [options]\n";
        std::cout << desc << "\n";
    } else if (vm.count("version")) {
        std::cout << "Apr 17th 2012\n";
