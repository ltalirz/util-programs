...
bool square(types::String cubeFile);

int main(int ac, char* av[]) {

    po::variables_map args;
    if (parse(ac, av, args)) {
        square(args["cubefile"].as< types::String >());
    }
...
bool square(types::String cubeFile) {
    ...
    c.grid.squareValues();
    String outCubeFile = "squared.";
    outCubeFile += io::getFileName(cubeFile);
    std::cout << "Writing squared cube file to " << outCubeFile << "\n";
...
bool parse(int ac, char* av[], po::variables_map& vm) {
    ...
    if (    !vm.count("cubefile")) {
        std::cout << "Usage: cubesquare [options]\n";
        std::cout << desc << "\n";
